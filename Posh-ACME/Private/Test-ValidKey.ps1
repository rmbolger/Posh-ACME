function Test-ValidKey {
    ...
    if ($Key -is [Security.Cryptography.RSA] -and ($Key.KeySize -lt 2048 -or $Key.KeySize -gt 4096)) {
        if ($ThrowOnFail) {
            throw [Management.Automation.ValidationMetadataException] "RSA key size out of range. Must be 2048-4096."
        }
        return $false
    }

    # For EC keys, LE's current Boulder server only supports P-256 and P-384, but may support P-521 by the time ACMEv2 is
    # production ready. In all testing so far, the KeySize matches the "P-xxxx" curve name. So we'll just use that to test
