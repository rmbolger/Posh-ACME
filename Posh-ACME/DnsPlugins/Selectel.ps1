function Add-DnsTxtSelectel {
    ...
    .EXAMPLE
        ...
        PS C:\>Add-DnsTxtSelectel '_acme-challenge.example.com' 'txt-value' $token

        Adds the specified TXT record with the specified value using a secure token.

    .EXAMPLE
        Add-DnsTxtSelectel '_acme-challenge.example.com' 'txt-value' 'my-token'

        Adds the specified TXT record with the specified value using a plaintext token.
    #>
}

function Remove-DnsTxtSelectel {
    [CmdletBinding(DefaultParameterSetName = 'Secure')]
    param(
        [Parameter(Mandatory, Position = 0)]
        [string]$RecordName,
    ...
    .EXAMPLE
        $token = Read-Host -Prompt "Selectel Token" -AsSecureString
        PS C:\>Remove-DnsTxtSelectel '_acme-challenge.example.com' 'txt-value' $token

        Removes the specified TXT record with the specified value using a secure token.

    .EXAMPLE
        Remove-DnsTxtSelectel '_acme-challenge.example.com' 'txt-value' 'my-token'

        Removes the specified TXT record with the specified value using a plaintext token.
    #>
}

function Save-DnsTxtSelectel {
    [CmdletBinding()]
    param(
        [Parameter(ValueFromRemainingArguments)]
        $ExtraParams
