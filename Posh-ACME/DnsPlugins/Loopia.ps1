function Add-DnsTxtLoopia {
    ...
    .EXAMPLE
        ...
        Add-DnsTxtLoopia '_acme-challenge.example.com' 'txt-value' 'user@loopiaapi' $pass

        Adds the specified TXT record with the specified value using a secure password.

    .EXAMPLE
        Add-DnsTxtLoopia '_acme-challenge.example.com' 'txt-value' 'user@loopiaapi' 'pass-value'

        Adds the specified TXT record with the specified value using a plaintext password.
    #>
}

function Remove-DnsTxtLoopia {
    [CmdletBinding(DefaultParameterSetName='Secure')]
    param(
        [Parameter(Mandatory,Position=0)]
        [string]$RecordName,
    ...
    .EXAMPLE
        $pass = Read-Host -Prompt "Loopia API password" -AsSecureString
        Remove-DnsTxtLoopia '_acme-challenge.example.com' 'txt-value' 'user@loopiaapi' $pass

        Removes the specified TXT record with the specified value using a secure password.

    .EXAMPLE
        Remove-DnsTxtLoopia '_acme-challenge.example.com' 'txt-value' 'user@loopiaapi' 'pass-value'

        Removes the specified TXT record with the specified value using a plaintext password.
    #>
}

function Save-DnsTxtLoopia {
    [CmdletBinding()]
    param(
        [Parameter(ValueFromRemainingArguments)]
        $ExtraParams
